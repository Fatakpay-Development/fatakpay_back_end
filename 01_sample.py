...
class BackendAutomation():
	...
	def readTheExcelFile(self):
		df = pd.read_excel(self.required_dataset['file_path'], engine="openpyxl", sheet_name=self.required_dataset['sub_sheet'])
	...
	def processFileData(self, dataset):
		...
		dataset['Request Data'] = dataset['Request Data'].replace("--application_id", self.user_dataset['application_id'])
		...
		if dataset['Method'] == "POST" or dataset['Method'] == "PUT":
			...
			if not files:
				...
				response = requests.request(dataset['Method'], dataset['API'], data=payload, headers=headers, files=files)
		else:
			payload = json.loads(dataset['Request Data'])
		...
		if "call-api?source=LMS&datapoint=get_accepted_offers&endpoint" in dataset['API'] and response.status_code == 200:
			response_data = response.json()
	...
	def codeForDebug(self):
		dataset = {}
		dataset['File Path'] = '[{"file_path": "C:/Users/shubh/Downloads/front_aadhar.jpg", "key_name": "file_front"}, {"file_path":"C:/Users/shubh/Downloads/back_aadhar.jpg","key_name":"file_back"}]'
